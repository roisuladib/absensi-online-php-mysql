        ...
        <?php if (isset($_GET['m'])) { ?>
          <?php if ($_GET['m'] == 'sa') { ?>
            <div class="alert alert-success alert-dismissible text-white fade show" role="alert">
              ...
              <span class="alert-text">Berhasil Menambah Wali Kelas</span>
          ...
          <?php } elseif ($_GET['m'] == 'ea') { ?>
            <div class="alert alert-danger alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text">Gagal Menambah Wali Kelas</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          <?php } elseif ($_GET['m'] == 'dd') { ?>
            <div class="alert alert-danger alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text"><strong>Wali Kelas!</strong> Sudah Ada</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          <?php } elseif ($_GET['m'] == 'se') { ?>
            <div class="alert alert-success alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text">Berhasil Mengubah Wali Kelas</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          <?php } elseif ($_GET['m'] == 'ee') { ?>
            <div class="alert alert-danger alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text">Gagal Mengubah Wali Kelas</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          <?php } elseif ($_GET['m'] == 'sd') { ?>
            <div class="alert alert-success alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text">Berhasil Menghapus Wali Kelas</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          <?php } elseif ($_GET['m'] == 'ed') { ?>
            <div class="alert alert-danger alert-dismissible text-white fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-satisfied"></i></span>
              <span class="alert-text">Gagal Menghapus Wali Kelas</span>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
          ...
          <table class="table align-items-center mb-0 table-borderless table-hover" id="dataTable">
            <thead>
              <tr>
                ...
                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">Wali Kelas</th>
            ...
            <tbody>
              <?php
              $no         = 0;
              $wali_Kelas = mysqli_query($db, 'SELECT * FROM wali_kelas');
              while ($wk  = mysqli_fetch_array($wali_Kelas)) {
                $classes  = mysqli_query($db, "SELECT * FROM kelas WHERE id = '" . $wk['kelas_id'] . "'");
                $class    = mysqli_fetch_array($classes);
                $teachers = mysqli_query($db, "SELECT * FROM guru WHERE id = '" . $wk['guru_id'] . "'");
                ...
                <tr>
                  ...
                  <td class="align-middle w-20">
                    <div class="d-flex w-100 h-100 justify-content-center align-items-center">
                      <a href="#" data-bs-target="#edit" data-id="<?= $wk['id'] ?>" id="editLesson" data-bs-toggle="modal" title="Hapus <?= $teacher['nama']; ?> ?" class="icon icon-shape btn bg-gradient-info p-2 icon-sm border-radius-lg text-center m-0 me-1 d-flex align-items-center justify-content-center cursor-pointer">
                      ...
                      <a href="../controllers/WaliKelas/destroy.php?id=<?= $wk['id']; ?>" onClick="return confirm('Apakah Kode <?= $teacher['nama']; ?>, Yakin Hapus ?')" title="Hapus <?= $teacher['nama']; ?> ?" class="icon icon-shape btn bg-gradient-danger p-2 icon-sm border-radius-lg text-center m-0 ms-1 d-flex align-items-center justify-content-center cursor-pointer">
      ...
      <div class="modal-body p-0">
        <div class="card card-plain">
          <div class="card-header pb-0 px-3 pt-3 text-center">
            <h3 class="font-weight-bolder text-primary text-gradient mb-0">Tambah Wali Kelas</h3>
          </div>
          <div class="card-body p-3">
            <form role="form text-left" action="../controllers/WaliKelas/add.php" method="POST">
              <label class="m-0">Kelas</label>
  ...
  <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-body p-0">
        <div class="card card-plain">
          <div class="card-header pb-0 px-3 pt-3 text-center">
            <h3 class="font-weight-bolder text-primary text-gradient mb-0">Edit Wali Kelas</h3>
          </div>
          <div class="card-body p-3">
            <form role="form text-left" action="../controllers/WaliKelas/update.php" method="POST">
              <input type="hidden" name="id" id="editWaliKelasId">
              <label class="m-0">Kelas</label>
              <div class="input-group mb-2">
                <select class="form-control" name="kelas_id" id="editWaliKelasKelas">
                  <option selected disabled>~ Pilih Kelas ~</option>
                  <?php
                  $classes      = mysqli_query($db, "SELECT * FROM kelas");
                  while ($class = mysqli_fetch_array($classes)) {
              ...
              <label class="m-0">Guru</label>
              <div class="input-group mb-2">
                <select class="form-control" name="guru_id" id="editWaliKelasGuru">
                  <option selected disabled>~ Pilih Guru ~</option>
                  <?php
                  $teachers      = mysqli_query($db, "SELECT * FROM guru");
                  while ($teacher = mysqli_fetch_array($teachers)) {
